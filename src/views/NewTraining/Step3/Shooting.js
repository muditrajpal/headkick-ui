...
const ShootingListContainer = styled(Column)`
  width: 100%;
...
const Shooting = ({ trainingData, onChange }) => (
  <Container>
    <ShootingListContainer>
      {[1, 2, 3, 4, 5, 6].map((rr) => (
        <InfoList>
            ...
            onChange={(e, { checked }) => {
              if (checked) {
                trainingData.drills.push({
                  name: `Shooting Drill ${r}`,
                  type: "Shooting",
                });
              } else {
                trainingData.drills = trainingData.drills.filter(
                  (rr) => rr.name !== `Shooting Drill ${r}`
                );
              }
